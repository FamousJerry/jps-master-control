...
const Clients: React.FC = () => {
  ...
  return (
    <Section title="Client Central">
      <div className="mb-4">
        <Button onClick={() => setEditing({ ...defaultClient })}>Add Client</Button>
      </div>
        ...
        <div className="border border-gray-700 rounded p-4 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Select
                value={editing.industry || "TV"}
                onChange={(e) => setEditing({ ...editing, industry: e.target.value })}
              >
                <option>TV</option><option>Film</option><option>Music Video</option>
                <option>Commercial</option><option>Other</option>
              </Select>
            </div>
            <div>
      ...
      <div className="space-y-2">
        {list.map((r)=>(
          <div key={r.id} className="bg-gray-800 p-3 rounded flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button className="px-3 py-1 rounded bg-gray-700"
                onClick={()=>setEditing({ id:r.id, name:r.name||"", amount:r.amount||0, stage:r.stage||"Lead" })}>Edit</button>
      ...
      <div className="space-y-2">
        {list.map((b)=>(
          <div key={b.id} className="bg-gray-800 p-3 rounded flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button className="px-3 py-1 rounded bg-gray-700"
                onClick={()=>setEditing({ id:b.id, title:b.title||"", start:b.start||"", end:b.end||"" })}>Edit</button>
